...
Two bugs live here, and they are the same bug seen from two ends — Recallix
treated the provider's speaker label as if it were a Recallix speaker number.
...
records carry a speaker each; Recallix read only the label on the parent
...
AssemblyAI names voices "A", "B", "C"… and Recallix rendered them by alphabet
...
class SpeakerIdentity:
    ...
    `raw` is the provider's, `key` is Recallix's stable internal handle, and
...
def is_generic_cluster(token: str) -> bool:
    ...
    any number Recallix could invent. Users renaming a speaker themselves sit
