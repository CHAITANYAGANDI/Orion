...
@pytest.mark.asyncio
async def test_draft_includes_the_actions(llm):
...
async def test_draft_contains_only_supplied_facts(llm):
    ...
    brief = _brief(action_items=["Ship on Tuesday."], key_points=[])
...
async def test_empty_sections_are_omitted(llm):
    brief = _brief(action_items=[], key_points=[])
    draft = await llm.draft_followup_email(brief)
...
async def test_key_points_only_used_as_a_fallback(llm):
    ...
    with_actions = await llm.draft_followup_email(_brief())
    assert "Key points:" not in with_actions.body

    without = await llm.draft_followup_email(
        _brief(action_items=[], key_points=["Ship on Tuesday."])
    )
...
def test_draft_endpoint_shape(client):
    ...
    assert body["subject"]
    assert "Ana: benchmark (due Thu)" in body["body"]
