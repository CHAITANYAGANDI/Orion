...
2. The second person to speak displayed as **Speaker 4**, because the provider
   had clustered them as "D" and Recallix decoded the letter by its position in
   the alphabet.
...
words — it was Recallix discarding the word-level labels on the way past, which
...
because the provider does not often produce one. It is the shape Recallix has to
...
def test_a_one_word_interjection_survives_as_its_own_turn():
    ...
    and labelled that utterance A, but attributed the fifth word to B. Recallix
...
def test_the_trace_shows_raw_beside_canonical():
    ...
    If the provider said B and this says Speaker 1, the fault is Recallix's. If
