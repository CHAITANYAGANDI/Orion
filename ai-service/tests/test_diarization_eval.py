...
* **B. Recallix parsing** — what ships today before any repair. Identical to A
  by construction, and that is the finding rather than a gap in the test: the
  parser is faithful, so where the provider is wrong Recallix is wrong.
...
def _provider_answers(fixture: Fixture) -> list[str | None]:
    """System A and B: the provider's labels, which Recallix passes through."""
...
SYSTEMS = {
    ...
    "B. Recallix today": _provider_answers,
...
def test_the_new_path_beats_every_other_system_overall():
    ...
    for other in ("A. AAI raw", "B. Recallix today", "C. SpeakerRefiner"):
        assert new > attribution(other), f"{other} scored {attribution(other):.3f} vs {new:.3f}"


def test_recallix_today_is_exactly_the_provider():
    ...
    for fixture in ALL:
        ...
        b = rows[("B. Recallix today", fixture.name)]
