...
-- created by the Postgres image (`reverie`) is a superuser, so connecting as it
...
--
--   reverie      owns the schema and runs Flyway. Superuser, needed for
--                 CREATE EXTENSION vector. Never serves a request.
--   reverie_app  what Spring and the ai-service connect as at runtime.
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'reverie_app') THEN
        CREATE ROLE reverie_app LOGIN PASSWORD 'reverie_app'
            NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE NOINHERIT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'reverie_sys') THEN
        CREATE ROLE reverie_sys LOGIN PASSWORD 'reverie_sys'
...
ALTER ROLE reverie_app NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE;
ALTER ROLE reverie_sys NOSUPERUSER BYPASSRLS   NOCREATEDB NOCREATEROLE;
...
-- so neither role can drop a policy to escape its tenant — and reverie_sys,
