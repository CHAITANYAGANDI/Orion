...
/**
 * The one place a message leaves Reverie.
 *
 ...
 *
 * <p>Off unless {@code RESEND_API_KEY} and {@code REVERIE_MAIL_FROM} are both
...
public class Mailer {
     ...
     * dead code that reads as a fallback. The same trap the reverie.auth-mode
     * comment in that file describes.
     */
    public Mailer(@Value("${reverie.mail.api-key:}") String apiKey,
                  @Value("${reverie.mail.from:}") String from,
                  @Value("${reverie.mail.reply-to:}") String replyTo,
                  @Value("${reverie.mail.base-url:https://api.resend.com}") String baseUrl) {
        ...
        if (!this.enabled) {
            log.info("Mail is off: set RESEND_API_KEY and REVERIE_MAIL_FROM to switch it on. "
