 ...
 *
 * <p>A deadline is the only thing Recallix extracts that carries a clock. A
 ...
 * Recallix on that day, they are in their calendar and their inbox. Of
...
public class TaskReminderJob {
    ...
    public TaskReminderJob(MeetingActionItemRepository items,
                           AccountMail mail,
                           @Value("${orion.mail.task-reminders:true}") boolean enabled) {
        this.items = items;
        this.mail = mail;
        this.enabled = enabled;
    }

    @Scheduled(cron = "${orion.mail.task-reminder-cron:0 0 8 * * *}", zone = "UTC")
    public void send() {
        if (!enabled) {
            return;
        }
