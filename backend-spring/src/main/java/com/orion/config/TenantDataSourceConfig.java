 ...
 *
 * <ul>
 *   <li><b>tenant</b> — {@code orion_app}, no BYPASSRLS. Serves every user
 *       request, and row-level security applies to it.</li>
 *   <li><b>system</b> — {@code orion_sys}, BYPASSRLS. Serves only the paths
...
public class TenantDataSourceConfig {
    ...
    DataSource systemDataSource(
            @Value("${spring.datasource.url}") String url,
            @Value("${orion.datasource.system.username:orion_sys}") String username,
            @Value("${orion.datasource.system.password:orion_sys}") String password) {
        HikariDataSource ds = pool(url, username, password);
        ds.setMaximumPoolSize(SYSTEM_POOL_MAX);
        ds.setPoolName("orion-system");
        return ds;
    }
