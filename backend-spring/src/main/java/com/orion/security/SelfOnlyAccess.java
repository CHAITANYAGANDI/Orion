 ...
 *
 * <p>{@code ORION_MAIL_SELF_ONLY} began as a declaration — "this deployment has
 ...
 *
 * <p>So the declaration is enforced. {@code ORION_MAIL_SELF_ONLY=true} requires
 * {@code ORION_MAIL_SELF_USER_ID}, and any other authenticated subject is
 ...
 * anything: leaving {@code ORION_MAIL_SELF_ONLY} unset — its default — asks for
 ...
 * account is perfectly valid, and Orion would provision a local user for it on
 * the first request. The only place that cannot be walked around is the point
 * where Orion decides a subject deserves a row — see
 * {@link com.orion.service.UserService#provision}.
 *
...
public class SelfOnlyAccess {
    ...
    public SelfOnlyAccess(@Value("${orion.mail.self-only:false}") boolean selfOnly,
                          @Value("${orion.mail.self-user-id:}") String selfUserId) {
        ...
        if (enforced && allowed.isEmpty()) {
            ...
            throw new IllegalStateException(
                    "ORION_MAIL_SELF_ONLY is true but ORION_MAIL_SELF_USER_ID is blank. "
                            ...
                            + "restricted. Set ORION_MAIL_SELF_USER_ID to your Clerk user id "
                            + "(it starts with `user_`, and is on your user in the Clerk "
                            + "dashboard), or unset ORION_MAIL_SELF_ONLY if you did not want "
                            + "the restriction.");
        }
        if (enforced) {
