 ...
 * hours after something looked wrong — and nothing in Recallix reads it for any
...
public class OutboxPurge {
    ...
    public OutboxPurge(OutboxEventRepository repo,
                       @Value("${recallix.outbox.keep-published-days:7}") int keepPublishedDays) {
     ...
     */
    @Scheduled(cron = "${recallix.outbox.purge-cron:0 30 4 * * *}", zone = "UTC")
