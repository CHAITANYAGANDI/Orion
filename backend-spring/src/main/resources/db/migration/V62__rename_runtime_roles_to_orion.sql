-- Renames the two runtime roles from recallix_* to orion_*, following the
-- product rename.
--
-- WHY THIS IS A MIGRATION AND NOT AN EDIT
--
-- The roles are named in the comments of ten earlier migrations, and those are
-- left exactly as they were. Flyway checksums every applied migration and
-- validates them on start-up, so changing one byte of V11 -- even inside a
-- comment -- makes the backend refuse to boot against a database that has
-- already run it. Those files are a record of what was done, not a description
-- of what is true now; read them as history and read this file for the current
-- names.
--
...
-- now creates orion_app and orion_sys directly, so the guards below find
...
-- query, which is after migration. So the pool that authenticates as orion_app
-- never races the statement that creates that name.

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'recallix_app')
       AND NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'orion_app') THEN
        ALTER ROLE recallix_app RENAME TO orion_app;
        RAISE NOTICE 'Renamed role recallix_app to orion_app.';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'recallix_sys')
       AND NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'orion_sys') THEN
        ALTER ROLE recallix_sys RENAME TO orion_sys;
        RAISE NOTICE 'Renamed role recallix_sys to orion_sys.';
    END IF;
END $$;

-- DELIBERATELY NOTHING ELSE.
--
-- An earlier draft of this migration also restated the role attributes here --
-- ALTER ROLE orion_app NOSUPERUSER NOBYPASSRLS ... -- as cheap belt-and-braces
-- against a role that had arrived by some other route. It is not cheap, and it
-- failed on the first run against Neon:
--
